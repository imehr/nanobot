...
class AgentLoop:
    ...
    def __init__(
        ...
        from nanobot.config.schema import BrowserToolConfig, ExecToolConfig, ToolsConfig, WebToolsConfig
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            if is_subagent and self._persist_subagent_followup(session, msg):
                self.sessions.save(session)
            self._set_tool_context(channel, chat_id, session.key, msg.metadata.get("message_id"))
        ...
        # Capture mode must run BEFORE extract_documents so PDFs and other
        # document media are handed to the knowledge service intact instead of
        # being text-extracted into the message body.
        ...
        if capture_mode and self.knowledge_service is not None:
            ...
            if capture_text or not results:
                results.append(
        ...
        self._set_tool_context(msg.channel, msg.chat_id, session.key, msg.metadata.get("message_id"))
        if message_tool := self.tools.get("message"):
            if isinstance(message_tool, MessageTool):
